...
import { createPortfolio, listPortfolio, removePortfolio, updatePortfolio } from './portfolio';
...
function installStorage(initial: Record<string, unknown> = {}) {
  ...
  return {
    values,
    restore: () => {
      if (previous === undefined) delete root.browser;
      else root.browser = previous;
    },
  };
}
...
describe('portfolio facade', () => {
  ...
  test('rejects invalid entries and indices without changing storage', async () => {
    const fixture = installStorage();
    try {
      expect(await createPortfolio({ title: 'Bad', skills: [], tags: [], url: 42 as never })).toBe(
        false,
      );
      expect(await updatePortfolio(0, { title: 'Missing', skills: [], tags: [], url: null })).toBe(
        false,
      );
  ...
  test('returns empty list when storage is unavailable', async () => {
    ...
    expect(await createPortfolio({ title: 'Project', skills: [], tags: [], url: null })).toBe(
      false,
    );
  });
});
